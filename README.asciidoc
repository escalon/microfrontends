...
=== Component Assembly
Svelte components can be assembled from other components. They can use `{{#if ..condition}}` tags to decide which components should be chosen.
...
Be wary with caching when delivering personalized content in your html. You must not deliver a page containing sensitive data if it has a non-personalized URL, even if you cache only static parts in your system and assemble them immediately before delivery. You have no control over caches between your site and the user's browser and you cannot prevent some proxy or a browser to deliver cached content for a user who was not the original recipient if the same URL produces different content for different users.
...
The other component should possibly provide a pkg.svelte or pkg['svelte.root'] entry in its package.json. That allows to bundle it by recompiling the raw component file. When compiling components from other packages that contain styles that need to be preprocessed, e.g. sass, it will be necessary to add the required preprocessor to your build.
