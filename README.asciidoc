...
== Todo
* dynamically choose choose product panel per product type, actions on action panel - with #if means new actions must be added statically before they can be used (is it possible to do it fully dynamically at all? What will be in the DOM?), and choosing the available actions should be up to the services team which writes the launchpad. Two level of ifs:
...
* each panel must retrieve its own data both in SSR and browser when activated
* styling conceptsüp+
** Master style vs. no master style but common patternlab
+
...
+
Without master style the pattern lab reigns only over parts which want to be reigned (e.g. classes describing the parts of the page and style which only applies to those). Components can choose to apply the pattern lab by building their own css prefixed with their own signature marker (svelte-xyx attribute).
+
TODO: How can I build css for a component which makes use of a patternlab but applies it only to the component?
...
=== Component Assembly
Components can be assembled from other components. They can use `#if` tags to decide which components should be chosen dynamically.
